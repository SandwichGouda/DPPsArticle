...
\textbf{Remark \cnt.} We conclude this section by a final remark and result. A look at the proof of Theorem 15 hints that
\begin{equation}
...
We provide an element of answer to the very last question in the form of a sufficient condition in the following result. For the rest, these questions are open. Note that two of these claims would imply the third one via the triangular inequality for the distance $ \mathcal W_{KR} $.

\textbf{Proposition \cnt.} Truncating the restricted Bergman process to a number of points $ N_R $ such that
\[
