...
\]
See \cite{ZerosGaussianPowerSeries} and \cite{Krishnapur} for a thorough study.
...
\[
\langle \phi_n^R, \phi_m^R \rangle_{L^2(\mathcal{B}(0,R))} = \sqrt{\frac{(n+1)(m+1)}{\pi}} \frac{1}{R^{n+m+2}} \int_{\mathcal{B}(0,R)} z^n \bar{z}^m dz.
\]
However,
\[
\int_{\mathcal{B}(0,R)} z^n \bar{z}^m dz = \int_0^R \int_0^{2\pi} r^{n+m} e^{i\theta(n-m)} r dr d\theta = 2\pi \delta_{n,m} \int_0^R r^{n+m+1} dr = 2\pi \delta_{n,m} \frac{R^{2n+2}}{2n+2}.
\]
...
We have (see \cite{DecreusefondMoroz2021}) $\lambda_n^R = \frac{\gamma(n+1, R^2)}{n!}$ where $\gamma$ stands for the incomplete gamma function $\gamma(n,x) = \int_0^x t^{n-1}e^{-t}dt$.
...
\[
\langle \phi_n^{r,R}, \phi_m^{r,R} \rangle_{L^2(T(r,R))} = \frac{1}{\pi} \sqrt{\frac{(n+1)(m+1)}{\lambda_n^{r,R} \lambda_m^{r,R}}} \int_{T(r,R)} z^n \bar{z}^m dz
\]
However,
\[
\int_{T(r,R)} z^n \bar{z}^m dz = \int_r^R \int_0^{2\pi} r^{n+m} e^{i\theta(n-m)} d\theta rdr = 2\pi \delta_{n,m} \int_r^R r^{n+m+1} dr = 2\pi \delta_{n,m} \left[ \frac{R^{2n+2}}{2n+2} - \frac{r^{2n+2}}{2n+2} \right]
\]
Consequently,
\[
...
\[
\lambda_n^A = (2n+2)\int_A r^{2n+1} dr,
\] 
...
\textbf{Remark \cnt.} Here we have thus constructed restrictions of the Bergman DPP to domains of the form $ Z([0,R]) $ and $ Z([r,R]) $. These restrictions are very interesting because as opposed to the original Bergman process, they exhibit a finite number of points almost surely. However, the main "loss" induced by such restrictions, clearly, is that we lack the possibility to simulate points in the annulus $ Z([1-\varepsilon,1]) $. But, given Theorem 25 the observations of Figure 1, it seems that it's really a shame since the points seem to be extremely concentrated on the border. A natural idea would be to restrict the Bergman DPP to a region of the form $ Z(A) $ such that $ [1-\varepsilon, 1] \subset A$ instead of $ \mathcal B(0,R) = Z([0,R]) $ for instance. Recalling that simulating an infinite number of points is unfeasible, this approach is quite doomed to failure as we show in the next result.

\textbf{Theorem \cnt.} The restriction of the Bergman DPP to any region that contains $ T(1-\varepsilon,1) = Z([1-\varepsilon,1]) $ exhibits an infinite number of points almost surely.

\textit{Proof.} 

As explained in Remark 31, such restriction would yield the eigenvalues
\[
\lambda_n^A = \int_A r^{2n+1} \mathrm d r .
\]
However, we would thus have 
\[
\lambda_n^A \ge \int_{[1-\varepsilon, 1]} r^{2n+1} \mathrm d r = \frac{1}{2n+2} - \frac{(1-\varepsilon)^{2n+2}}{2n+2}.
\]
The series $ \displaystyle \sum_{n\ge 0} \frac{(1-\varepsilon)^{2n+2}}{2n+2} $ is convergent, hence the sum $ \displaystyle \sum_{n \ge 0} \lambda_n^A $ is not. As a result, the corresponding integral operator is not trace class, and so the corresponding DPP exhibits an infinite number of points almost surely. \qed

\textbf{Remark \cnt.} It is interesting to note that despite last theorem, it is possible to consider regions such that "touch" (in the sense of the topological closure) the unit circle (as opposed to those included in $ Z([0,R]) $ for some $ R < 1 $). 

\textbf{Theorem \cnt.} Consider real numbers $ \alpha \in (0,1) $ and $ \beta > 1 $. Let 
\[
    A_{\alpha, \beta} := \bigcup_{k\geqslant 0} \left[ 1-\frac{1}{\beta^k}, \frac{1}{\beta^k} + \alpha \left( \frac{1}{\beta^k} - \frac{1}{\beta^{k+1}} \right)  \right].
\]
Then, as opposed to the regions $ Z([0,R]) $ and $ Z([r, R])$ studied before, the regions $Z(A_{\alpha, \beta})$ verify simultaneously the two following properties :

(i) For all $ \alpha \in (0,1) $ and all $ \beta > 1 $, the restricted Bergman operator to $ A_{\alpha, \beta} $ is trace class, and the corresponding DPP hence exhibits a finite number of points almost surely.

(ii) For all $ \beta > 1 $, the Lebesgue measure of $ Z(A_{\alpha, \beta}) $ tends to that of $ \mathcal B(0,1) $ as $ \alpha \to 1^- $.

\textit{Proof.}

The eigenvalues of the restricted operator write as 
\begin{align*}
    \lambda_n^{\alpha, \beta} &= \int_{A_{\alpha, \beta}} r^{2n+1} \mathrm d r \\
    &= \sum_{n = }\int_{A_{\alpha, \beta}} r^{2n+1} \mathrm d r \\
\end{align*}
